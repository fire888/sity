...
function SF_scripts(){
	...
	$(".forget-form").submit(function(event) {
		...
		console.log($('.password').val() !== $('.password-repeated').val());
		if ($('.password').val() !== $('.password-repeated').val()) {
			console.log('Wrong passwords');
			$(".alert-form-error .message").html("Пароли не совпадают");
			$(".alert-form-error").fadeIn(200).delay(10000).fadeOut(200);
			return true;
		}

		console.log('Submitting:',form[0].name);
		var	term = form.serialize(),
			url = api_host + form.attr("action"),
		...
		if(required_fields_filled){
			...
			posting
				.done(function(data){
					...
					$('.alert-forget-form-success').modal('show');
				})
				.fail(function(data){
					console.log(data);
					$('#forget_password').modal('hide');
			...
			if(required_fields_filled){
				...
				.fail(function(data){
					...
					if (data.responseText === 'Unauthorized') {
						data.responseText = 'Учетная запись не найдена, проверь вводимые данные или пройди регистрацию'
					}
					$(".alert-form-error .message").html(data.responseText);
					$(".alert-form-error").fadeIn(200).delay(10000).fadeOut(200);
					hidegRecaptchaPopup();
				});
			}else{
	...
	$(".registration-form").submit(function(event){
		...
		function submitRegistrationForm(form){
			...
			if(required_fields_filled){
				...
				posting
					.done(function(data){
						$(".alert-registration-form-success").fadeIn(200).delay(10000).fadeOut(200);
					...
					.fail(function(err){
						if (err.status === 500) {
							$(".alert-registration-form-error").fadeIn(200).delay(10000).fadeOut(200);
